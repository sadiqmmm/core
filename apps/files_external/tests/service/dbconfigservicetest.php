...
class DBConfigServiceTest extends TestCase {
	...
	public function testGetMountsFor() {
		...
		$this->assertEquals($id, $mounts[0]['id']);
	...
	public function testGetAdminMountsForWithGlobalAndUser() {
		$id1 = $this->addMount('/test', 'foo', 'bar', 100, DBConfigService::MOUNT_TYPE_ADMIN);
		$id2 = $this->addMount('/test2', 'foo2', 'bar2', 100, DBConfigService::MOUNT_TYPE_ADMIN);
		$id3 = $this->addMount('/test3', 'foo3', 'bar3', 100, DBConfigService::MOUNT_TYPE_PERSONAl);
		...
		$this->assertEquals($id1, $mounts[0]['id']);
		$this->assertEquals([['type' => DBConfigService::APPLICABLE_TYPE_GLOBAL, 'value' => null, 'mount_id' => $id1]], $mounts[0]['applicable']);
		$this->assertEquals($id2, $mounts[1]['id']);
		$this->assertEquals([['type' => DBConfigService::APPLICABLE_TYPE_USER, 'value' => 'test', 'mount_id' => $id2]], $mounts[1]['applicable']);
	}
}
